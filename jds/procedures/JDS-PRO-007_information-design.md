...
1. The logo SVG source is `jds/assets/logo.svg`. Colour variants are generated by `scripts/logo-variants.py` into `jds/assets/logo-variants/category/` (by document category) and `jds/assets/logo-variants/domain/` (by engineering domain).
