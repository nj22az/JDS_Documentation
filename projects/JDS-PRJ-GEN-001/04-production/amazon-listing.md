# Amazon KDP — Listing & Metadata

> Sales copy and metadata for *Städa i Garaget*. Written for **amazon.se** (Swedish market).
> Paste the description into KDP's description field — KDP accepts basic HTML (bold, lists,
> headings). The plain-text version below it is for reference.

---

## Title fields

- **Book title:** Städa i Garaget
- **Subtitle:** Rensa, organisera och håll ordning i verkstad och förråd
- **Author:** Nils Johansson
- **Series:** Städa-serien (Book 1) — *optional; only set once book 2 is planned*
- **Edition:** 1
- **Language:** Swedish

---

## Book description (HTML — paste into KDP)

```html
<h2>Äntligen ett garage där bilen får plats — och allt har sin plats.</h2>
<p>Du öppnar garageporten, letar fem minuter efter en skruvmejsel, hittar den inte, och
stänger porten med en suck. Bilen står ute. Igen. <b>Det är inte ditt fel — och det går att
fixa.</b></p>
<p>De flesta städböcker stannar vid tröskeln till garaget. Det är synd, för garaget är ofta
det rum som stjäl mest tid, pengar och dåligt samvete i hela huset — fullt av verktyg, däck,
färg och en livstid av "kanske behöver jag det".</p>
<p><i>Städa i Garaget</i> tar med sig den ordningsmetod som proffs använder i verkstäder och
maskinrum — och översätter den till vanlig svenska och till ditt eget garage. Allt vilar på
fem enkla steg:</p>
<p><b>Sortera · Systematisera · Städa · Standardisera · Säkra</b></p>
<p>Det här är ingen bok om att storstäda en gång (det håller i tre veckor, och det vet du
redan). Det är en bok om att bygga ett <b>system</b> som håller ordningen åt dig.</p>
<h3>I boken får du:</h3>
<ul>
<li>En tydlig metod i fem steg — ett steg per helg, klar på ungefär fem helger</li>
<li>Ett <b>Helgprojekt</b> i varje kapitel: konkret, gör-det-nu, ofta klart på en förmiddag</li>
<li>Knep för att hitta vilket verktyg som helst på tre sekunder</li>
<li>Säker hantering av bensin, färg och kemikalier — det farliga som inte får gå i soporna</li>
<li>En 10-minutersrutin som håller garaget i ordning året om</li>
<li>Särskilda kapitel för fritidshusets uthus, lägenhetsförrådet och att ärva en verkstad</li>
</ul>
<p>Inga dyra prylar. Inga specialverktyg. Bara en metod som fungerar — skriven av någon som
faktiskt har stått på verkstadsgolvet.</p>
<p><b>Res dig ur soffan, öppna porten, och ta tillbaka ditt garage.</b></p>
```

### Book description (plain text — reference)

Äntligen ett garage där bilen får plats — och allt har sin plats.

Du öppnar garageporten, letar fem minuter efter en skruvmejsel, hittar den inte, och stänger
porten med en suck. Bilen står ute. Igen. Det är inte ditt fel — och det går att fixa.

De flesta städböcker stannar vid tröskeln till garaget. Det är synd, för garaget är ofta det
rum som stjäl mest tid, pengar och dåligt samvete i hela huset. *Städa i Garaget* tar med sig
den ordningsmetod som proffs använder i verkstäder — och översätter den till ditt eget garage,
i fem enkla steg: Sortera, Systematisera, Städa, Standardisera, Säkra. Ett steg per helg, klart
på ungefär fem helger. Res dig ur soffan, öppna porten, och ta tillbaka ditt garage.

---

## Keywords (7 — KDP allows 7 keyword fields)

1. städa garaget
2. ordning och reda hemma
3. förvaring garage verkstad
4. rensa och organisera
5. döstädning gör det själv
6. hemorganisering bok svenska
7. förråd och fritidshus ordning

> Rationale: mixes high-intent niche terms ("städa garaget", "förvaring garage verkstad")
> with proven adjacent categories ("ordning och reda", "döstädning") to ride existing demand.

## Categories (KDP lets you pick up to 3)

1. Böcker > Hem & trädgård > Hem & hushåll
2. Böcker > Hobby, fritid & livsstil > Gör-det-själv
3. Böcker > Rådgivning & livsstil > Personlig utveckling & självhjälp

> If amazon.se category names differ at upload, pick the closest "Home & Garden / Home
> Improvement" and "Crafts, Hobbies & Home / DIY" equivalents.

## Pricing (suggestion)

| Format | List price (SEK) | Notes |
|--------|------------------|-------|
| Kindle eBook | 49–69 kr | 70% royalty band starts at ~35 kr; impulse-buy price point |
| Paperback (later) | 149–179 kr | Once a print interior PDF is produced |

## A+ Content ideas (optional, boosts conversion)

- A banner image of the five steps (Sortera → Säkra) as a simple icon row
- A before/after spread (the same pegboard wall, messy vs ordered)
- A "what's inside" module listing the 15 chapters
- An author module with the engineer/5S credibility line

## Look Inside optimisation

KDP's preview shows the opening. The book is ordered so the **Inledning** (the hook) comes
right after the title/copyright pages — that is deliberate: the first thing a browsing buyer
reads is the emotional hook and the engineer/5S promise, not a dry table of contents.
