# Regulatory Traceability Matrix — AFS 2017:3

| | |
|---|---|
| **Document No.** | — (supplemental reference) |
| **Revision** | B |
| **Date** | 2026-04-11 |
| **Status** | CURRENT |
| **Author** | Nils Johansson |
| **Regulation** | AFS 2017:3 — Use and Inspection of Pressurised Equipment |

---

## 1. Purpose

This document maps every requirement in the JDS Ongoing Maintenance Program (JDS-PRO-010) back to the specific sections of **AFS 2017:3** (Swedish regulation for use and inspection of pressurised equipment) that it satisfies.

JDS-PRO-010 is the authoritative working procedure. This matrix is the evidence trail that proves the procedure meets regulatory requirements. The two documents are deliberately separate: the procedure is clean and operational; this matrix is the compliance reference.

---

## 2. How to Use This Document

- **During audits:** Show this matrix to demonstrate that every regulatory requirement is covered
- **When updating PRO-010:** Check this matrix to ensure no regulatory requirement is lost
- **When adapting to other countries:** Create a new traceability matrix mapping to the other country's regulation

---

## 3. Traceability — Equipment Inventory

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (Swedish) | Regulatory Requirement (English) |
|---|---|---|---|
| Step 2: Build Equipment Register | Kap. 2, §5 | Brukaren ska ha en förteckning | The operator shall maintain a register of all pressurised equipment |
| Record vessel identification data | Kap. 2, §5 punkt 1–5 | Förteckningen ska innehålla... | The register shall contain: identification, location, classification, inspection status, safety devices |
| Photograph every nameplate | Kap. 2, §6 | Märkning ska vara läsbar | Markings shall be legible and maintained |

---

## 4. Traceability — Classification

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (Swedish) | Regulatory Requirement (English) |
|---|---|---|---|
| Step 3: Classify every vessel | Kap. 3, Bilaga 2 | Klassindelning av trycksatta anordningar | Classification of pressurised equipment into Class A / B |
| Determine fluid group | PED 2014/68/EU Art. 13 | Grupp 1 / Grupp 2 | Group 1 (dangerous) / Group 2 (other) |
| PS × V threshold > 50 bar·L | Kap. 3, §8 | Tillämpningsområde | Scope — equipment above threshold requires classification |
| Record PED category | AFS 2016:1 / PED Annex II | Kategori I–IV eller Art. 4.3 | Category based on pressure, volume, and medium |

---

## 5. Traceability — Inspection Intervals

> **Important correction (Rev B):** Inspection intervals are NOT fixed values. They are determined by the accredited inspection body based on equipment condition per Bilaga 1. The values below are base intervals that may be extended or shortened.

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (English) |
|---|---|---|
| Class A/B: Driftprov at base intervals | 5 Kap. §4, Bilaga 1 §1.4.1 | Recurring operational test: 2 years (general) or 4 years (air/nitrogen, refrigeration, LPG, vacuum-insulated) |
| Class A: Int./ext. examination — condition-based | 5 Kap. §4, Bilaga 1 §2.2 | Interval set by inspection body: 6 months to 10 years based on equipment condition |
| Class B: Recurring inspection = driftprov only | 5 Kap. §4, para. 3 | Class B requires only driftprov, no mandatory int./ext. examination |
| Extended driftprov interval (max 4 years) | Bilaga 1 §1.4.2 | If safety equipment functioned without action at 2 previous tests |
| Shortened driftprov interval (halved) | Bilaga 1 §1.4.3 | If safety equipment did NOT function at previous test |
| Book inspectors 3 months ahead | Best practice | Not a regulatory requirement; JDS operational standard |

---

## 6. Traceability — Who May Inspect

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (Swedish) | Regulatory Requirement (English) |
|---|---|---|---|
| Accredited Inspector for Class A | Kap. 4, §14 | Ackrediterat kontrollorgan | Accredited inspection body (SWEDAC accredited) |
| Competent Person for Class B | Kap. 4, §17 | Egen kontroll av den som brukar | Own inspection by operator with documented competence |
| Competence documentation | Kap. 2, §4 | Kompetenskrav | Competence requirements for persons performing work on pressurised equipment |

---

## 7. Traceability — Ongoing Supervision

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (Swedish) | Regulatory Requirement (English) |
|---|---|---|---|
| Routine supervision walk-arounds | Kap. 2, §3 | Fortlöpande tillsyn | Ongoing supervision — the operator's continuous responsibility |
| Visual checks between inspections | Kap. 2, §3 | Tillsyn ska ske regelbundet | Supervision shall be performed regularly |
| Document supervision rounds | Kap. 2, §5 | Dokumentation av tillsyn | Supervision shall be documented |

---

## 8. Traceability — Findings and Actions

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (Swedish) | Regulatory Requirement (English) |
|---|---|---|---|
| Approved → continue operation | Kap. 4, §19 | Godkänd | Equipment may continue in service |
| Approved with remarks → monitor | Kap. 4, §19 | Godkänd med anmärkning | Equipment may continue with conditions; findings must be addressed |
| Rejected → out of service | Kap. 4, §20 | Underkänd | Equipment shall be taken out of service immediately |
| Act on findings before return to service | Kap. 2, §7 | Åtgärda brister | Deficiencies shall be remedied before continued use |
| Report incidents | Kap. 2, §8 | Anmälningsplikt | Duty to report overpressure events, failures, dangerous occurrences to Arbetsmiljöverket |

---

## 9. Traceability — Safety Devices

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (Swedish) | Regulatory Requirement (English) |
|---|---|---|---|
| Test safety devices regularly | Kap. 5, §21 | Säkerhetsventiler ska provas | Safety valves shall be tested at defined intervals |
| Record test results | Kap. 5, §22 | Dokumentation av provning | Test results shall be documented |
| Set pressure within design limits | Kap. 5, §21 | Inställningstryck | Relief pressure shall not exceed vessel design pressure |

---

## 10. Traceability — Records and Documentation

| JDS Requirement (PRO-010) | AFS 2017:3 Section | Regulatory Requirement (Swedish) | Regulatory Requirement (English) |
|---|---|---|---|
| Maintain equipment register | Kap. 2, §5 | Förteckning | Equipment register shall be maintained and kept current |
| Keep inspection certificates | Kap. 4, §19 | Kontrollintyg | Inspection certificates shall be kept on file |
| Keep EU Declarations of Conformity | AFS 2016:1, §9 | EU-försäkran om överensstämmelse | EU Declaration of Conformity shall be available |
| Retain records per JDS-QMS-002 | Kap. 2, §5 | Bevaras så länge anordningen används | Records shall be kept for the life of the equipment |

---

## 11. Swedish Terminology Reference

This section provides the Swedish-to-English mapping for regulatory terms encountered in AFS 2017:3. JDS documents use the English equivalents exclusively.

| Swedish Term | JDS English Term | Context |
|---|---|---|
| Fortlöpande tillsyn | Ongoing supervision / Ongoing maintenance | The operator's continuous maintenance responsibility |
| Tryckkärl | Pressure vessel | Closed vessel designed to hold fluid under pressure |
| Brukare | Operator | Person or organisation responsible for the equipment |
| Ackrediterat kontrollorgan | Accredited inspection body | Third-party inspector (DEKRA, Kiwa, RISE, etc.) |
| Egen kontroll | Own inspection | Inspection by operator's competent personnel |
| Utvändig undersökning | External examination | Visual inspection of vessel exterior |
| Invändig undersökning | Internal examination | Examination of vessel interior |
| Tryckprov | Pressure test | Hydrostatic or pneumatic proof test |
| Kontrollintyg | Inspection certificate | Official document issued after inspection |
| Kontrollplan | Inspection plan | Annual schedule of inspections and supervision |
| Tillsynsprotokoll | Supervision checklist | Record of routine walk-around supervision |
| Godkänd | Approved | Inspection result — vessel may continue in service |
| Godkänd med anmärkning | Approved with remarks | Inspection result — findings require attention |
| Underkänd | Rejected | Inspection result — vessel must be taken out of service |
| Klass A / Klass B | Class A / Class B | Risk classification determining inspection requirements |
| Säkerhetsventil | Safety valve | Pressure relief device |
| Arbetsmiljöverket | Swedish Work Environment Authority | National regulatory authority |
| SWEDAC | Swedish Board for Accreditation | Accredits inspection bodies |

---

## Revision History

| Rev | Date | Author | Description |
|-----|------|--------|-------------|
| A | 2026-03-25 | N. Johansson | Initial release — full traceability mapping of JDS-PRO-010 to AFS 2017:3 |
| B | 2026-04-11 | N. Johansson | Corrected inspection intervals: changed from incorrect fixed values (24/72/144 mo) to condition-based intervals per Bilaga 1. Added driftprov extension/shortening rules. |
